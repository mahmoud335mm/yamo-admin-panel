...
describe("5D-2R evidence-review", () => {
  ...
  it("reviewer can mark evidence clean", async () => {
    const { admin, env } = await getVerifiedAdminClient();
    void sessions; void env;
    const res: any = await (sessions.support?.client ?? admin).rpc('review_recharge_dispute_evidence', { _evidence_id: '00000000-0000-0000-0000-000000000000', _decision: 'clean' });
