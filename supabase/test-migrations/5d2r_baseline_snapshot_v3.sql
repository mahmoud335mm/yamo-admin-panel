...
--
-- Phase 5D-2R baseline snapshot correction v3.
-- Removes references to relations that are not part of the current schema
-- (refund_receivables, wallet_adjustments, refund_gateway_attempts) and uses
-- real current-schema relations where available.

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.test_environment_marker WHERE environment_name = 'test') THEN
    RAISE EXCEPTION 'TEST_PROJECT_MARKER_MISSING: baseline snapshot v3 refused';
  END IF;
END $$;
