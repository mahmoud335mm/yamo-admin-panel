...
insert into public.yamo_message_sections(section_key,label_ar,label_en,icon_key,sort_order,enabled,system_section) values
...
begin
  perform public.yamo_admin_require('messages.read');
  select coalesce(jsonb_agg(to_jsonb(s)-'updated_by' order by s.sort_order,s.created_at),'[]'::jsonb) into result from public.yamo_message_sections s;
  return result;
end $$;
...
begin
  ...
  insert into public.yamo_message_sections(section_key,label_ar,label_en,icon_key,sort_order,enabled,system_section,updated_by)
  values(section_id,left(trim(coalesce(p_section->>'label_ar','قسم')),80),left(trim(coalesce(p_section->>'label_en','Section')),80),left(coalesce(p_section->>'icon_key','message'),40),coalesce((p_section->>'sort_order')::integer,100),coalesce((p_section->>'enabled')::boolean,true),coalesce((p_section->>'system_section')::boolean,false),admin_id)
  on conflict(section_key) do update set label_ar=excluded.label_ar,label_en=excluded.label_en,icon_key=excluded.icon_key,sort_order=excluded.sort_order,enabled=excluded.enabled,updated_at=now(),updated_by=admin_id;
  select to_jsonb(s)-'updated_by' into result from public.yamo_message_sections s where s.section_key=section_id;
  perform public.yamo_admin_log('messages.section_upsert','message_section',section_id,null,result,'إضافة أو تعديل قسم الرسائل');
  return result;
end $$;
...
begin
  ...
  delete from public.yamo_message_sections where section_key=p_section_key and system_section=false;
  get diagnostics deleted_count=row_count;
  if deleted_count>0 then perform public.yamo_admin_log('messages.section_delete','message_section',p_section_key,null,null,'حذف قسم مخصص من الرسائل'); end if;
