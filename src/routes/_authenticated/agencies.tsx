...
function AgenciesPage() {
  ...
  const refresh = () => {
    ...
    qc.invalidateQueries({ queryKey: ["yamo-agency-finance-config"] });
    qc.invalidateQueries({ queryKey: ["yamo-agency-dashboard"] });
  ...
  return <div className="space-y-6" dir="rtl">
    ...
    </div><div className="flex gap-2"><CreateRootDialog agencies={tree.data??[]} onDone={refresh}/><Button variant="outline" onClick={refresh}><RefreshCw className="ml-2 h-4 w-4"/>تحديث</Button></div></div>
...
function CreateRootDialog({agencies,onDone}:{agencies:TreeAgency[];onDone:()=>void}) {
  ...
  const m=useMutation({mutationFn:async()=>{const {error}=await db.rpc("admin_create_yamo_agency_v2",{p_owner_legacy_id:owner,p_name:name,p_parent_id:parent==="ROOT"?null:parent,p_duration_days:Number(days),p_permanent:permanent,p_country_code:country,p_whatsapp:whatsapp});if(error)throw error;},onSuccess:()=>{toast.success("تم فتح الوكالة وتوليد الكودين");setOpen(false);onDone();},onError:(e:Error)=>toast.error(e.message)});
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button><Plus className="ml-2 h-4 w-4"/>فتح وكالة</Button></DialogTrigger><DialogContent dir="rtl"><DialogHeader><DialogTitle>فتح وكالة رئيسية أو فرعية</DialogTitle></DialogHeader><div className="grid gap-3"><Field label="ID مالك الوكالة" value={owner} set={setOwner}/><Field label="اسم الوكالة" value={name} set={setName}/><AgencySelect label="الوكالة الأم" value={parent} set={setParent} agencies={agencies} allowRoot/>{parent!=="ROOT"&&<><Field label="مدة التبعية بالأيام" value={days} set={setDays} type="number"/><label className="flex items-center justify-between rounded-md border p-3"><span>ربط دائم</span><Switch checked={permanent} onCheckedChange={setPermanent}/></label></>}<Field label="كود الدولة (اختياري)" value={country} set={setCountry}/><Field label="واتساب (اختياري)" value={whatsapp} set={setWhatsapp}/></div><DialogFooter><Button variant="outline" onClick={()=>setOpen(false)}>إلغاء</Button><Button disabled={!owner.trim()||!name.trim()||m.isPending} onClick={()=>m.mutate()}>{m.isPending&&<Loader2 className="ml-2 h-4 w-4 animate-spin"/>}إنشاء وتوليد الأكواد</Button></DialogFooter></DialogContent></Dialog>;
}
