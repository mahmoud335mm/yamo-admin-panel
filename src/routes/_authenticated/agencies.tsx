...
function sourceLabel(v:string){const x=v.toLowerCase();return x.includes("message")?"الرسائل المدفوعة":x.includes("call")?"المكالمات المدفوعة":x.includes("room")&&x.includes("gift")?"هدايا الغرف":(x.includes("private")||x.includes("1v1")||x.includes("one_to_one"))&&x.includes("gift")?"هدايا واحد ضد واحد":x.includes("gift")?"الهدايا الخاصة":x.includes("task")?"مكافآت المهام":x.includes("recharge")?"عمولة الشحن":x.includes("bonus")?"مكافآت إضافية":v}
function actionLabel(v:string){return v.replaceAll("agency.","وكالة: ").replaceAll("host.","مضيف: ").replaceAll("_"," ")}
function HostMembers({agencyId,hosts,onDone}:{agencyId:string;hosts:any[];onDone:()=>void}){
  const[reason,setReason]=useState("تعديل عضوية المضيف من مركز الوكالات");
  const cards=useQuery({queryKey:["agency-host-full-cards",agencyId],queryFn:async()=>{const{data,error}=await db.rpc("admin_get_yamo_agency_host_cards",{p_agency_id:agencyId});if(error)throw error;return (data??[]) as any[];}});
  const rows=cards.data??hosts;
  const m=useMutation({mutationFn:async(h:any)=>{const{error}=await db.rpc("admin_set_host_removed",{p_agency_id:agencyId,p_user_id:h.user_id,p_removed:!h.removed_at,p_reason:reason});if(error)throw error;},onSuccess:async()=>{toast.success("تم تحديث المضيف");await cards.refetch();onDone()},onError:(e:Error)=>toast.error(e.message)});
  return <div className="space-y-3"><div className="flex gap-2"><Input value={reason} onChange={e=>setReason(e.target.value)} placeholder="سبب العملية"/><Button size="sm" variant="outline" disabled={cards.isFetching} onClick={()=>cards.refetch()}>{cards.isFetching?<Loader2 className="ml-1 h-4 w-4 animate-spin"/>:<RefreshCw className="ml-1 h-4 w-4"/>}تحديث الأعضاء</Button></div>
  {cards.error&&<div className="rounded-md border border-destructive/30 p-3 text-xs text-destructive">{(cards.error as Error).message}</div>}
  <div className="grid gap-3 lg:grid-cols-2">{rows.map(h=><Card key={h.user_id} className={h.removed_at?"opacity-60":""}><CardContent className="space-y-3 p-4"><div className="flex items-center gap-3"><UserPhoto src={h.avatar_url} name={h.name} size="h-16 w-16"/><div className="min-w-0 flex-1"><b className="block truncate text-base">{h.name}</b><div className="text-xs text-muted-foreground" dir="ltr">ID {h.legacy_id}</div><div className="mt-1 flex gap-1"><Badge variant={h.removed_at?"secondary":"default"}>{h.removed_at?"تمت الإزالة":"مضيف نشط"}</Badge><Badge variant="outline">{h.account_status??"active"}</Badge></div></div><Button size="sm" variant={h.removed_at?"outline":"destructive"} disabled={reason.trim().length<3||m.isPending} onClick={()=>m.mutate(h)}>{h.removed_at?"إعادة العضوية":"إزالة"}</Button></div>
  <div className="grid grid-cols-3 gap-2"><Balance label="كوينز الحساب" value={h.coins} tone="text-orange-600"/><Balance label="لؤلؤ الحساب" value={h.pearls} tone="text-violet-600"/><Balance label="أرباح الوكالة" value={h.total_earned_pearls??h.earned_pearls} tone="text-emerald-600"/></div>
  <div><div className="mb-2 text-xs font-semibold">مصادر الربح</div><div className="grid grid-cols-2 gap-2">{Object.entries(h.earnings_by_source??{}).map(([source,amount])=><div key={source} className="flex items-center justify-between rounded-md border bg-muted/30 px-3 py-2 text-xs"><span>{sourceLabel(source)}</span><b dir="ltr">{Number(amount||0).toLocaleString("en-US")}</b></div>)}{Object.keys(h.earnings_by_source??{}).length===0&&<div className="col-span-2 rounded-md border border-dashed p-3 text-center text-xs text-muted-foreground">لا توجد أرباح مسجلة لهذا المضيف</div>}</div></div>
  <div className="border-t pt-2 text-xs text-muted-foreground"><div>انضم: <b dir="ltr">{new Date(h.joined_at).toLocaleString("ar-EG-u-nu-latn")}</b></div><div>الطريقة: {h.joined_via==="app_code"?"كود من التطبيق":h.joined_via==="admin"?"إضافة من اللوحة":h.joined_via??"قديم"} · الكود: <b dir="ltr">{h.joined_code??"—"}</b></div><div>نهاية المهمة: {h.task_ends_at?new Date(h.task_ends_at).toLocaleDateString("ar-EG-u-nu-latn"):"دائم"}</div></div></CardContent></Card>)}{!cards.isFetching&&!rows.length&&<div className="col-span-full rounded-lg border border-dashed p-8 text-center text-sm text-muted-foreground">لا يوجد مضيفون</div>}</div></div>
}
function Balance({label,value,tone}:{label:string;value:any;tone:string}){return <div className="rounded-lg border p-2 text-center"><div className={`font-bold ${tone}`} dir="ltr">{Number(value||0).toLocaleString("en-US")}</div><div className="text-[11px] text-muted-foreground">{label}</div></div>}
