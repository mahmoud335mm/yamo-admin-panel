...
export const Route = createFileRoute("/_authenticated/charging-pearl-transfers")({
  component: () => (
    <PermissionGuard permission="charging_pearl_transfers.read">
      <ChargingPearlTransfersPanel />
    </PermissionGuard>
  ),
});

export function ChargingPearlTransfersPanel() {
  ...
  const list = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        ...
        .select("id, reference, amount, status, created_at, from_user_id, to_user_id, note", {
          count: "exact",
        })
  ...
  const reverse = useMutation({
    mutationFn: async () => {
      if (!reverseId) return;
      const { error } = await supabase.rpc("reverse_pearl_transfer", {
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["cpearl_transfers"] });
      setReverseId(null);
      setReason("");
    },
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">تحويلات اللؤلؤ</h1>
        <p className="text-sm text-muted-foreground">تحويلات اللؤلؤ بين الحسابات.</p>
      </div>
      <Card>
              ...
              <TableBody>
                {list.data!.rows.map((r) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell>
                      {r.status === "completed" && has("charging_pearl_transfers.reverse") && (
      ...
      <Dialog open={!!reverseId} onOpenChange={(o) => !o && setReverseId(null)}>
        <DialogContent dir="rtl">
          <DialogHeader>
            <DialogTitle>عكس تحويل اللؤلؤ</DialogTitle>
          </DialogHeader>
          <div>
          ...
          <DialogFooter>
            ...
            <Button
              variant="destructive"
              disabled={reason.length < 5 || reverse.isPending}
              onClick={() => reverse.mutate()}
            >
              {reverse.isPending && <Loader2 className="ml-1 h-4 w-4 animate-spin" />}تأكيد
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
