  ...
  const list = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        ...
        .select(
          "id, member_role, status, assigned_at, user_id, agency_id, charging_agencies:agency_id(name, display_id), profiles:user_id(legacy_id, display_name)",
          { count: "exact" },
        )
      ...
      if (q.trim()) {
        ...
        rows = rows.filter((r) => {
          const p = r.profiles as { legacy_id?: string; display_name?: string } | null;
          return (
            p?.legacy_id?.toLowerCase().includes(t) || p?.display_name?.toLowerCase().includes(t)
          );
        });
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">وكلاء الشحن</h1>
          <p className="text-sm text-muted-foreground">تفعيل مستخدم كوكيل شحن وربطه بوكالة.</p>
        </div>
                    ...
                    <TableRow key={r.id}>
                      <TableCell>
                        ...
                        <div className="font-mono text-xs text-muted-foreground">
                          {p?.legacy_id ?? r.user_id.slice(0, 8)}
                        </div>
                      </TableCell>
                      <TableCell>
                        <Link
                          to="/charging-agencies/$id"
                          params={{ id: r.agency_id }}
                      ...
                      <TableCell>
                        <Link to="/charging-agents/$id" params={{ id: r.user_id }}>
...
function ActivateDialog({ onDone }: { onDone: () => void }) {
  ...
  const activate = useMutation({
    ...
    onSuccess: () => {
      toast.success("تم تفعيل الوكيل");
      qc.invalidateQueries({ queryKey: ["charging_agents"] });
      setOpen(false);
      onDone();
    },
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button>
          <Plus className="ml-1 h-4 w-4" /> تفعيل وكيل
        </Button>
      </DialogTrigger>
      <DialogContent dir="rtl">
        <DialogHeader>
          <DialogTitle>تفعيل مستخدم كوكيل شحن</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <div>
            <Label>UID المستخدم</Label>
            <Input value={uid} onChange={(e) => setUid(e.target.value)} placeholder="YMU-000123" />
          </div>
          <div>
            <Label>الوكالة</Label>
            <Select value={agencyId} onValueChange={setAgencyId}>
              <SelectTrigger>
                <SelectValue placeholder={agencies.isLoading ? "…" : "اختر"} />
              </SelectTrigger>
              <SelectContent>
                {(agencies.data ?? []).map((a) => (
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={!uid || !agencyId || activate.isPending}
          >
            {activate.isPending && <Loader2 className="ml-1 h-4 w-4 animate-spin" />}تفعيل
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
