...
function Page() {
  ...
  const profile = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("profiles")
        .select("id, legacy_id, display_name, country, status")
        .eq("id", id)
        .maybeSingle();
  ...
  const memberships = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(
          "id, member_role, status, assigned_at, agency_id, charging_agencies:agency_id(name, display_id)",
        )
        .eq("user_id", id)
  ...
  const suspend = useMutation({
    mutationFn: async (reason: string) => {
      const { error } = await supabase.rpc("suspend_charging_agent", {
        _user_id: id,
        _reason: reason,
      } as never);
      if (error) throw error;
  ...
  if (!profile.data)
    return <div className="py-16 text-center text-muted-foreground">المستخدم غير موجود</div>;
  const p = profile.data;
  const active = memberships.data?.find((m) => m.status === "active");
  const coinBalance = wallet.data?.coins ?? 0;
  const pearlBalance = wallet.data?.pearls ?? 0;
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="font-mono text-xs text-muted-foreground">{p.legacy_id ?? id.slice(0, 8)}</p>
        </div>
        <div className="flex gap-2">
          {active?.status === "active" && has("charging_agents.suspend") && (
            <ReasonBtn
              label="تعليق الوكيل"
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <Stat
          title="الوكالة"
          value={(active?.charging_agencies as { name?: string } | null)?.name ?? "-"}
        />
      </div>
        ...
        <TabsContent value="settings">
          <Card>
            <CardContent className="grid grid-cols-2 gap-3 pt-4 text-sm sm:grid-cols-3">
              ...
              <KV
                k="تبديل لؤلؤ→كوينز"
                v={settings.data?.can_exchange_pearls_to_coins ? "نعم" : "لا"}
              />
            </CardContent>
          </Card>
        </TabsContent>
          ...
          <Card>
            <CardContent className="pt-4">
              {(memberships.data ?? []).length === 0 ? (
                <div className="py-6 text-center text-sm text-muted-foreground">لا عضويات</div>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {memberships.data!.map((m) => {
                      const ag = m.charging_agencies as {
                        name?: string;
                        display_id?: string;
                      } | null;
                      return (
          ...
          <Card>
            <CardContent className="pt-4">
              {(coinTxns.data ?? []).length === 0 ? (
                <div className="py-6 text-center text-sm text-muted-foreground">لا تحويلات</div>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
