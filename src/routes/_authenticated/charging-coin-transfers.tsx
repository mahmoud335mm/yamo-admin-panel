...
export const Route = createFileRoute("/_authenticated/charging-coin-transfers")({
  component: () => (
    <PermissionGuard permission="charging_coin_transfers.read">
      <ChargingCoinTransfersPanel />
    </PermissionGuard>
  ),
});

export function ChargingCoinTransfersPanel() {
  ...
  const list = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        ...
        .select(
          "id, reference, amount, status, created_at, agent_user_id, recipient_user_id, agency_id, note",
          { count: "exact" },
        )
  ...
  const reverse = useMutation({
    mutationFn: async () => {
      if (!reverseId) return;
      const { error } = await supabase.rpc("reverse_coin_transfer", {
        _transfer_id: reverseId,
        _reason: reverseReason,
      } as never);
      if (error) throw error;
    },
    onSuccess: () => {
      toast.success("تم عكس التحويل");
      qc.invalidateQueries({ queryKey: ["ccoin_transfers"] });
      setReverseId(null);
      setReverseReason("");
    },
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">تحويلات الكوينز</h1>
        <p className="text-sm text-muted-foreground">جميع تحويلات الكوينز من وكلاء الشحن.</p>
      </div>
      <Card>
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          >
            ...
            <SelectContent>
              ...
              {Object.entries(CHARGING_TXN_STATUS).map(([k, v]) => (
                  ...
                  {list.data!.rows.map((r) => (
                    <TableRow key={r.id}>
                      ...
                      <TableCell className="font-mono text-xs">
                        {r.agent_user_id.slice(0, 8)}
                      </TableCell>
                      <TableCell className="font-mono text-xs">
                        {r.recipient_user_id.slice(0, 8)}
                      </TableCell>
                      <TableCell>{fmtNum(r.amount)}</TableCell>
                      <TableCell>
                        <Badge variant={r.status === "completed" ? "default" : "secondary"}>
                          {CHARGING_TXN_STATUS[r.status] ?? r.status}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-xs">{fmtDate(r.created_at)}</TableCell>
                      <TableCell>
                        {r.status === "completed" && has("charging_coin_transfers.reverse") && (
                          <Button size="sm" variant="ghost" onClick={() => setReverseId(r.id)}>
      ...
      <Dialog open={!!reverseId} onOpenChange={(o) => !o && setReverseId(null)}>
        <DialogContent dir="rtl">
          <DialogHeader>
            <DialogTitle>عكس تحويل الكوينز</DialogTitle>
          </DialogHeader>
          <div>
            <Label>سبب العكس (5 أحرف على الأقل)</Label>
            <Textarea
              value={reverseReason}
          ...
          <DialogFooter>
            ...
            >
              {reverse.isPending && <Loader2 className="ml-1 h-4 w-4 animate-spin" />}تأكيد العكس
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
