...
export function ChargingFinancialControl({ agency }: { agency: Agency }) {
  ...
  if (!has("charging_finance.read")) return null;
  const target = agents.find((a) => a.user_id === pending?.userId);
  return <Card dir="rtl">
    <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2"><CardTitle>التحكم المالي وبيانات الوكالة</CardTitle><div className="flex gap-2"><Button variant="outline" onClick={() => refresh()} disabled={bundle.isFetching}>تحديث</Button>{has("charging_finance.manage") && <Button variant="outline" onClick={openEdit}>تعديل البيانات / إيقاف وتشغيل</Button>}</div></CardHeader>
    <CardContent className="space-y-5">
      ...
      <div className="grid gap-3 md:grid-cols-3"><div><Label>حساب الوكيل</Label><select className="mt-2 w-full rounded border bg-background p-2" value={selected?.user_id ?? ""} onChange={(e) => setUserId(e.target.value)}>{agents.map((a) => <option key={a.user_id} value={a.user_id}>{a.name} — {a.legacy_id}</option>)}</select></div><div><Label>قيمة الإضافة أو الخصم</Label><Input dir="ltr" inputMode="numeric" value={amount} onChange={(e) => setAmount(e.target.value.replace(/[^0-9]/g, ""))} /></div><div><Label>سبب العملية — إجباري</Label><Input value={reason} onChange={(e) => setReason(e.target.value)} /></div></div>
      ...
      <div className="flex flex-wrap gap-2">{has("charging_finance.credit") && <><Button onClick={() => prepare("coins", false)}>إضافة كوينز</Button><Button onClick={() => prepare("pearls", false)}>إضافة لؤلؤ</Button></>}{has("charging_finance.debit") && <><Button variant="destructive" onClick={() => prepare("coins", true)}>خصم كوينز</Button><Button variant="destructive" onClick={() => prepare("pearls", true)}>خصم لؤلؤ</Button></>}</div>
      <p className="text-xs text-muted-foreground">الرصيد النهائي يُفحص من السيرفر لحظة التنفيذ. لا تُحذف السجلات؛ تصحيح الأخطاء بعملية عكسية مستقلة.</p>
